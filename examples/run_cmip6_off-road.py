...
def clean_temp_output(f):
    ...
    ssp119 = df_avg[df_avg['scenario'] == 'ssp119'].copy().drop(columns='scenario')
    ...
    df_avg['CO2 vs. non-CO2'] = np.where(df_avg['scenario'].str.contains('CO2'), 'CO2', 'non-CO2')
    df_avg['SLCP vs. Kyoto'] = np.where(df_avg['scenario'].str.contains('CO2|CH4|N2O'), 'Kyoto', 'SLCP')

    df_avg['Sector'] = np.where(df_avg['scenario'].str.contains('AGR'), 'TRA_OT_AGR', '')
    df_avg['Sector'] = np.where(df_avg['scenario'].str.contains('RAI'), 'TRA_OT_RAI', df_avg['Sector'])
    df_avg['Sector'] = np.where(df_avg['scenario'].str.contains('CNS'), 'TRA_OT_CNS', df_avg['Sector'])
    df_avg['Sector'] = np.where(df_avg['scenario'].str.contains('OTHER'), 'TRA_OT_OTHER', df_avg['Sector'])

    df_avg['Species'] = df_avg['scenario'].str.split('_', expand=True)[1]
    ...
    # Filter to 1990 to 2050
    df_avg = df_avg[(df_avg['timebounds'] >= 1990) & (df_avg['timebounds'] <= 2050)]

    # Rename columns
    df_avg = df_avg.rename(columns={'timebounds': 'Year', 'Sector': 'Sub-Sector',
                                    'temp': 'Total Temperature Anomaly',
                                    'BAU_temp_attribution': 'BAU Sub-Sector/Species Attributable Warming',
                                    'Striving_temp_attribution': 'Striving Sub-Sector/Species Attributable Warming',
                                    'Avoided': 'Avoided Warming'})

    df_avg = df_avg[['scenario', 'CO2 vs. non-CO2', 'SLCP vs. Kyoto', 'Species', 'Sub-Sector', 'Year',
                     'Total Temperature Anomaly', 'Avoided Warming',
                     'BAU Sub-Sector/Species Attributable Warming', 'Striving Sub-Sector/Species Attributable Warming']]
...
def main():
    ...
    # Read input data
    forcing = pd.read_csv('../CMIP6/off_road/off_road_rcmip-radiative-forcing-annual-means-v5-1-0.csv')
    ems = pd.read_csv('../CMIP6/off_road/off_road_rcmip-emissions-annual-means-v5-1-0.csv')
    conc = pd.read_csv('../CMIP6/off_road/off_road_rcmip-concentrations-annual-means-v5-1-0.csv')
    ...
    # Save the results
    temp.to_csv('Results/off_road/temperature_all.csv', index=False)
