...
def interp_off_road(off_road_ems):
    ...
    existing_years = years  # Extract year columns
    ...
    year_columns = list(set(off_road_ems.columns) - set(['Scenario', 'Source', 'Region', 'Sector', 'Sub-Sector', 'Species', 'Units']))
    off_road_ems = off_road_ems[['Scenario', 'Region', 'Sector', 'Sub-Sector', 'Species', 'Units'] + year_columns].copy()
    ...
    off_road_ems = off_road_ems.melt(
        id_vars=['Scenario', 'Region', 'Sector', 'Sub-Sector', 'Species', 'Units'],
        var_name='Year',
        value_name='ems'
    )
...
def remove_domestic_aviation(off_road_ems):
    """
    ...
    data = {
        ...
        "Sector": ["off-road", "off-road", "off-road"],
        "Sub-Sector": ["TRA_OT_OTHER", "TRA_OT_OTHER", "TRA_OT_OTHER"],
        "Species": ["co2", "bc", "ch4"],
        "Units": ["Mt", "kt", "kt"],
        "Year": [2022, 2022, 2022],
        "ems": [392.016, 6.682, 11.184],
        "ems_domestic_aviation": [327.186258, 3.106199, 0.000000],
        "domestic_aviation_share": [0.834625, 0.464861, 0.000000],
    }

    off_road_bau = pd.DataFrame(data)
...
def add_gwp(off_road_export):
    # Calculate GWP
    gwp = pd.read_csv('gwp.csv')
    # Rename 'CO2', to 'co2', 'CH4' to 'ch4', 'N2O' to 'n2o', 'BC' to 'bc'
    gwp = gwp.rename(columns={'Pollutant': 'Species'}).drop(columns='Comments')
    gwp['Species'] = gwp['Species'].str.lower()

    # Merge GWP values
    off_road_export = off_road_export.merge(gwp, on='Species', how='left')
    # Calculate GWP
    off_road_export['GWP20'] *= off_road_export['ems']
    off_road_export['GWP100'] *= off_road_export['ems']
    # Where 'units are 'kt', convert to 'Mt'
    off_road_export.loc[off_road_export['Units'] == 'kt', ['GWP20', 'GWP100']] *= 1e-3
    # off_road_export.loc[off_road_export['Units'] == 'Mt', ['GWP20', 'GWP100']] *= 1e-3

    return off_road_export


def calc_off_road(off_road_ems):
    """
    ...
    For all other sub-sectors:
        ...
        BC: 2035: 60%, 2050: 100%
    """
    off_road_ems = off_road_ems.rename(columns={'Sector': 'Sub-Sector'})
    off_road_ems['Sector'] = 'off-road'
    
    
    # Drop the shipping sector ('TRA_OT_SHIP_NAT'), this is covered by the marine sector
    off_road_ems = off_road_ems[~(off_road_ems['Sub-Sector'].isin(['SUM', 'ALL SUM', 'TRA_OT_SHIP_NAT', 'TRA_OTH']))].copy()

    off_road_ems['Scenario'] = 'BAU'

    # Convert to long format and interpolate intermediate years
    off_road_ems = interp_off_road(off_road_ems)

    off_road_ems = remove_domestic_aviation(off_road_ems)
    ...
    off_road_export = add_gwp(off_road_export)

    off_road_export.to_csv('off-road_sub_sectors.csv', index=False)
...
def calculate_on_road_diesel_wtt(df_long):
    ...
    using the upstream emission factors from the GHS.
    """
    energy = pd.read_csv('on_road_PJ_2deg_gap.csv')
...
def calculate_off_road_diesel_wtt(df_long):
    ...
    if 'Sub-Sector' in non_road.columns:
        id_cols = ['Scenario', 'Region', 'Sector', 'Sub-Sector', 'Source', 'Units', 'Year']
    else:
        id_cols = ['Scenario', 'Region', 'Sector', 'Source', 'Units', 'Year']
...
def run():
    ...
    """
    co2 = pd.read_excel('Global non-road emissions_regional.xlsx', 'Non-road CO2_CLEAN')
    bc = pd.read_excel('Global non-road emissions_regional.xlsx', 'Non-road BC_CLEAN')
    ch4 = pd.read_excel('Global non-road emissions_regional.xlsx', 'Non-road CH4_CLEAN')
    ...
    off_road = pd.concat([co2, bc, ch4], ignore_index=True)
    
    off_road['Source'] = 'TTW'

    # off_road_long = off_road.melt(
    #     id_vars=['Species', 'Region', 'Sector', 'Source', 'Units'],
    #     var_name='Year',
    #     value_name='ems'
    # )

    df_long = calc_off_road(off_road)

    # Combine with other sectors
    other_sect = pd.read_csv('../SLCP_inventory_long.csv')
    other_sect = other_sect.loc[other_sect['Sector'] != 'off-road']

    # Convert everything to Mt
    other_sect.loc[other_sect['Units'] == 'Gt', 'ems'] *= 1000

    other_sect = add_gwp(other_sect)

    # Combine the two
    df_long = pd.concat([df_long, other_sect]).reset_index(drop=True)

    df_long.to_csv('regional_off-road_sub_sectors_with_all_sectors.csv', index=False)
